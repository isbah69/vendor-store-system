...
if (!in_array($_SESSION['role'], ['admin', 'sales man'])) {
    echo "Access Denied!";
    exit;
}
?>
        ...
        <div class="col-xs-12">
          ...
          <div class="box">
            ...
            <?php if ($_role === 'admin' || $_role === 'manager' || $_role === 'sales_man'): ?>
                <a href="add_sales_item.php" class="btn btn-success">Add New</a>
            <?php else: ?>
      ...
      if ($result && mysqli_num_rows($result) > 0) {
          while ($row = mysqli_fetch_assoc($result)) {
              ...
              if ($_role === 'admin' || $_role === 'manager') {
                  echo "<a href='edit_sales_item.php?id=".$row["sale_item_id"]."' class='btn btn-success btn-sm'>Edit</a> ";
              } else {
                  echo "<button class='btn btn-success btn-sm' disabled title='Access Denied' style='opacity:0.5;cursor:not-allowed;'>Edit</button> ";
              }
              if ($_role === 'admin') {
                  echo "<a href='delete_sales_item.php?id=".$row["sale_item_id"]."' class='btn btn-danger btn-sm' onclick=\"return confirm('Are you sure?');\">Delete</a>";
              } else {
