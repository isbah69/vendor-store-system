...
require_roles(['admin', 'manager','sales man', 'user']);
      ...
      <?php
        ...
        if (mysqli_num_rows($result) > 0) {
          while ($row = mysqli_fetch_assoc($result)) {
            ...
            echo "<td>";
            if ($_role === 'admin' || $_role === 'manager' || $_role === 'sales_man') {
                echo "<a href='edit_customer.php?id=".$row["customer_id"]."' class='btn btn-success btn-sm'>Edit</a> ";
            } else {
                echo "<button class='btn btn-success btn-sm' disabled title='Access Denied' style='opacity:0.5;cursor:not-allowed;'>Edit</button> ";
            }
            if ($_role === 'admin') {
                echo "<a href='delete_customer.php?id=".$row["customer_id"]."' class='btn btn-danger btn-sm' onclick=\"return confirm('Are you sure?');\">Delete</a>";
            } else {
