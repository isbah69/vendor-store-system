                    ...
                    if(mysqli_num_rows($result) > 0)
                    {
                      while ($row = mysqli_fetch_assoc($result)) 
                      {
                          ...
                          if ($_role === 'admin' || $_role === 'manager' || $_role === 'inventory_staff') {
                              echo "<a href='edit_category.php?id=".$row["category_id"]."' class='btn btn-success btn-sm'>Edit</a> ";
                          } else {
                              echo "<button class='btn btn-success btn-sm' disabled title='Access Denied' style='opacity:0.5;cursor:not-allowed;'>Edit</button> ";
                          }
                          if ($_role === 'admin') {
                              echo "<a href='delete_categories.php?id=".$row["category_id"]."' class='btn btn-danger btn-sm' onclick=\"return confirm('Are you sure?');\">Delete</a>";
                          } else {
