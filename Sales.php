...
require_roles(['admin', 'manager', 'sales_man', 'user']);
...
// Only admin and  Sales man can access products page
if (!in_array($_SESSION['role'], ['admin', 'sales man', 'user'])) {
    echo "Access Denied!";
    exit;
}
?>
      ...
      <div class="row">
        <div class="col-xs-12">
          ...
          <div class="box">
            ...
            <?php if ($_role === 'admin' || $_role === 'manager' || $_role === 'sales_man'): ?>
                <a href="add_sales.php" class="btn btn-success">Add New</a>
            <?php else: ?>
...
if (!$con) {
    echo "<tr><td colspan='5'>🚨 Connection Error: " . mysqli_connect_error() . "</td></tr>";
    exit;
}

$sql = "SELECT s.sale_id, c.name AS name, 
       s.sale_date, s.total_amount
FROM sales AS s
JOIN companies AS c ON s.company_id = c.company_id;
";
...
if (!$result) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        echo "<tr>";
        echo "<td>" . $i++ . "</td>";   // changed here
        echo "<td>" . htmlspecialchars($row['name']) . "</td>";
        echo "<td>" . htmlspecialchars($row['sale_date']) . "</td>";
        echo "<td>" . htmlspecialchars(number_format($row['total_amount'], 2)) . "</td>";
        echo "<td>";
        if ($_role === 'admin') {
            echo "<a href='edit_sales.php?id=".urlencode($row["sale_id"])."' class='btn btn-success btn-sm'>Edit</a> ";
        } else {
        ...
        if ($_role === 'admin' || $_role === 'manager') {
            echo "<a href='delete_sales.php?id=".urlencode($row["sale_id"])."' class='btn btn-danger btn-sm' onclick=\"return confirm('Are you sure?');\">Delete</a>";
        } else {
    ...
    echo "<tr><td colspan='5' class='text-center text-danger'>🤷 No sales found in the database.</td></tr>";
}
?>
  </tbody>
</table>
