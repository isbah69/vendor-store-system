...
require_roles(['admin', 'manager', 'inventory_staff', 'accountant']);
...
if (!in_array($_SESSION['role'], ['admin', 'manager', 'inventory_staff', 'sales man', 'accountant'])) {
    echo "Access Denied!";
    exit;
}
?>
      ...
      <div class="row">
        <div class="col-xs-12">
          ...
          <div class="box">
            ...
            <?php if ($_role === 'admin' || $_role === 'manager' || $_role === 'accountant'): ?>
                <a href="add_return.php" class="btn btn-success">Add New</a>
            <?php else: ?>
            ...
            s.total_amount
        FROM returns AS r
        LEFT JOIN sales AS s ON r.sale_id = s.sale_id
        ORDER BY r.return_id DESC";
                    ...
                    if(mysqli_num_rows($result) > 0)
                    {
                      while ($row = mysqli_fetch_assoc($result)) 
                      {
                          ...
                          echo "</td>";
                          echo "<td>";
                          echo $row["return_status"];
                          echo "</td>";
                          echo "<td>";
                          echo $row["total_refund"];
                          echo "</td>";
                          echo "<td>";
                          echo $row["sale_id"];
                          echo "</td>";
                         ...
                         echo "<td>";
                          if ($_role === 'admin' || $_role === 'manager' || $_role === 'accountant') {
                              echo "<a href='edit_returns.php?id=".$row["return_id"]."' class='btn btn-success btn-sm'>Edit</a> ";
                          } else {
                              echo "<button class='btn btn-success btn-sm' disabled title='Access Denied' style='opacity:0.5;cursor:not-allowed;'>Edit</button> ";
                          }
                          if ($_role === 'admin') {
                              echo "<a href='delete_returns.php?id=".$row["return_id"]."' class='btn btn-danger btn-sm' onclick=\"return confirm('Are you sure?');\">Delete</a>";
                          } else {
