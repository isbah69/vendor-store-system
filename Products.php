...
require_roles(['admin', 'sales man', 'manager', 'inventory_staff', 'user']);
...
if (!in_array($_SESSION['role'], ['admin', 'sales man',"manager", 'user', 'inventory_staff'])) {
    echo "Access Denied!";
    exit;
}
?>
...
// Use LEFT JOIN to include all products, even if brand or category is missing
$sql = "SELECT 
            p.product_id, 
        ...
        ORDER BY p.product_id ASC"; // optional, order by ID
...
if(mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        echo "<td>".($row["b_name"] ?? 'No Brand')."</td>";
        ...
        if ($_role === 'admin' || $_role === 'manager' || $_role === 'inventory_staff') {
            echo "<a href='edit_product.php?id=".$row["product_id"]."' class='btn btn-success btn-sm'>Edit</a> ";
        } else {
            echo "<button class='btn btn-success btn-sm' disabled title='Access Denied' style='opacity:0.5;cursor:not-allowed;'>Edit</button> ";
        }
        if ($_role === 'admin') {
            echo "<a href='delete_products.php?id=".$row["product_id"]."' class='btn btn-danger btn-sm' onclick=\"return confirm('Are you sure?');\">Delete</a>";
        } else {
